...
names = ["Флафи", "Фалафель", "Ведьмак", "Чипалина", "Пучеглазик", "Слайм", "Шустрый", "Следопыт",
         "Пушистый волк", "Субарик", "Т-Рекс", "Птенец", "Рядовой", "Вэлл", "Ветеран", "Геймер",
         "Самурай", "Милс Кел"]
generation = 0
...
def run_game(genomes, config):
    ...
    f = open(filename, "a")
    f.write(str(generation) + " " + str(game_speed) + " " + str(score) + "\n")
    generation += 1
    game_speed = 8.0
    score = 0
    score_speedup = 100
    ...
    while True:
        ...
        for i, enemy in enumerate(enemies):
            ...
            for j, dinosaur in enumerate(dinosaurs):
                if dinosaur.hitbox.colliderect(enemy.hitbox):
                    genomes[j][1].fitness -= 1  # lower fitness (failed)
                    dinosaurs.pop(j)
        ...
        for i in rem_list:
            ...
            for j, dinosaur in enumerate(dinosaurs):
                genomes[j][1].fitness += 10  # raise fitness (+5 for every enemy)
        ...
        if score > score_speedup:
            score_speedup += 100 * (game_speed / 20)
            game_speed += 1
        ...
        # display game speed
        score_label = score_font.render("Скорость: " + str(game_speed / 1) + "x", True, (50, 50, 50))
        score_label_rect = score_label.get_rect()
